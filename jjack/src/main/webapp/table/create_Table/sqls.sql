...
CREATE TABLE TB_Member (
	m_no NUMBER(10) CONSTRAINT m_no_pk PRIMARY KEY, -- 회원번호
	...
	CONSTRAINT mm_id_uk UNIQUE(m_id)
);
...
CREATE TABLE TB_Apply (
	a_no NUMBER(10) CONSTRAINT a_no_pk PRIMARY KEY, -- 신청인덱스 
	...
	CONSTRAINT a_cond_ck CHECK(a_cond BETWEEN 1 AND 10),
	CONSTRAINT am_no_fk FOREIGN KEY m_no REFERENCES TB_Member(m_no),
	CONSTRAINT ae_event_fk FOREIGN KEY e_eventdate REFERENCES TB_Event(e_eventdate)
);
...
CREATE TABLE TB_Guest (
	g_no NUMBER(10) CONSTRAINT g_no_pk PRIMARY KEY, -- 입소자인덱스 
	a_no NUMBER(10) NOT NULL, -- 신청인덱스
	g_gisoo NUMBER(5) NOT NULL, -- 기수 
	g_nick VARCHAR2(8) NOT NULL, -- 호칭 
	g_lunch NUMBER(10) CONSTRAINT g_lunch_fk REFERENCES TB_Guest(g_no), -- 도시락짝 
	g_choice NUMBER(10) CONSTRAINT g_choice_fk REFERENCES TB_Guest(g_no), -- 짝선택 
	g_course CHAR(1) CONSTRAINT g_course_ck CHECK(g_course IN ('A', 'B', 'C', 'D', 'S')), -- 추천코스
	CONSTRAINT ga_no_fk FOREIGN KEY(a_no) REFERENCES TB_Apply(a_no)
);
...
CREATE TABLE TB_Event (
	e_eventdate VARCHAR(8) CONSTRAINT e_eventdate_pk PRIMARY KEY, -- 행사날짜 
...
CREATE TABLE TB_Notice (
	n_no NUMBER(6) CONSTRAINT n_no_pk PRIMARY KEY, -- 공지번호 
	...
	CONSTRAINT n_IsShow_ck CHECK(n_IsShow IN ('Y', 'N')) 
);


-- 후기게시판
CREATE TABLE TB_Review (
	r_no NUMBER(10) CONSTRAINT r_no_pk PRIMARY KEY, -- 후기번호 
	...
	CONSTRAINT rg_no_fk FOREIGN KEY(g_no) REFERENCES TB_Guest(g_no),
	CONSTRAINT r_IsShow_ck CHECK(r_IsShow IN ('Y', 'N'))
);
...
CREATE TABLE TB_Comment (
	c_no NUMBER(20) CONSTRAINT c_no_pk PRIMARY KEY, -- 댓글인덱스 
	...
	CONSTRAINT c_IsShow_ck CHECK(c_IsShow IN ('Y', 'N')),
	CONSTRAINT cm_no_fk FOREIGN KEY(m_no) REFERENCES TB_Member(m_no),
	CONSTRAINT cr_no_fk FOREIGN KEY(r_no) REFERENCES TB_Review(r_no)
);
