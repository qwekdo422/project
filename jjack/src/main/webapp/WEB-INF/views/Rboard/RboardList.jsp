...
<body style="margin: 0 auto;">
	...
	<div class="container">
			...
			<table width="1100" border="1" align="center">
				...
				<c:forEach var="data" items="${RLIST}">
					<tr>
						<td>${data.rno}</td>
						<td class="title"><a
							href="../Rboard/rHitUpProc.do?rno=${data.rno}&nowPage=${nowPage}">${data.rtitle}</a></td>
						<td>기수?</td>
						<td>작성자?</td>
						<td>${data.rdate}</td>
						<td>짝?</td>
		...
		<table width="1000" align="center">
			<tr>
				<td align="center">
					<%--	[이전] --%> <c:if test="${nowPage ne 1}">
						<!--'nowPage=1 이 맨 앞으로' 니까nowPage가 1이 아닐 때만 보이게 하자.  -->
						<a href="../Rboard/RboardList.do?nowPage=1">[맨 앞으로]</a>
					</c:if> <c:if test="${(startPage eq 1 || startPage ne 1) && nowPage ne 1}">
						<%-- ★★ nowPage가 1페이지면 [이전] 버튼이 사라지는 로직--%>
						<a href="../Rboard/RboardList.do?nowPage=${nowPage -1}">[이전]</a>
					</c:if> <%--	[1][2][3] --%> <c:forEach var="page" begin="${startPage}"
						end="${endPage}">
						<a href="../Rboard/RboardList.do?nowPage=${page}">[${page}]</a>
					</c:forEach> <%--	[다음] --%> <c:if
						test="${endPage ne totalPage || nowPage ne totalPage}">
						<%--<c:if test="${endPage ne totalPage || endPage eq totalPage}"> --%>
						<a href="../Rboard/RboardList.do?nowPage=${nowPage + 1}">[다음]</a>
					</c:if> <c:if test="${nowPage ne totalPage}">
